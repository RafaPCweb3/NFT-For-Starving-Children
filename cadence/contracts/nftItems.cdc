...
pub contract nftItems: NonFungibleToken {
    ...
    // totalSupply
    // The total number of nftItems that have been minted
    //
    ...
    pub resource NFT: NonFungibleToken.INFT, MetadataViews.Resolver {
        ...
        pub fun name(): String {
            return nftItems.rarityToString(self.rarity)
                .concat(" ")
                .concat(nftItems.kindToString(self.kind))
        }

        pub fun description(): String {
            return "A "
                .concat(nftItems.rarityToString(self.rarity).toLower())
                .concat(" ")
                .concat(nftItems.kindToString(self.kind).toLower())
                .concat(" with serial number ")
                .concat(self.id.toString())
        }

        pub fun imageCID(): String {
            return nftItems.images[self.kind]![self.rarity]!
        }
    ...
    // This is the interface that users can cast their nftItems Collection as
    // to allow others to deposit nftItems into their Collection. It also allows for reading
    // the details of nftItems in the Collection.
    pub resource interface NftItemsCollectionPublic {
        pub fun deposit(token: @NonFungibleToken.NFT)
        pub fun getIDs(): [UInt64]
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT
        pub fun borrowNftItem(id: UInt64): &nftItems.NFT? {
    ...
    pub resource Collection: NftItemsCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic, MetadataViews.ResolverCollection {
        ...
        pub fun borrowViewResolver(id: UInt64): &AnyResource{MetadataViews.Resolver} {
            ...
            let nft_Item = nft as! &nftItems.NFT
            return nft_Item as &AnyResource{MetadataViews.Resolver}
        }
    ...
    pub resource NFTMinter {
        ...
        pub fun mintNFT(
            ...
            recipient.deposit(token: <-create nftItems.NFT(id: nftItems.totalSupply, kind: kind, rarity: rarity))
        ...
        pub fun addNewImagesForKind(from: AuthAccount, newImages: {Kind: {Rarity: String}}) {
            let kindValue = nftItems.images.containsKey(newImages.keys[0]) 
            if(!kindValue) {
                nftItems.images.insert(key: newImages.keys[0], newImages.values[0])
    ...
    // If an account does not have a nftItems.Collection, panic.
    // If it has a collection but does not contain the itemID, return nil.
    // If it has a collection and that collection contains the itemID, return a reference to that.
    //
    pub fun fetch(_ from: Address, itemID: UInt64): &nftItems.NFT? {
        let collection = getAccount(from)
            .getCapability(nftItems.CollectionPublicPath)!
            .borrow<&nftItems.Collection{nftItems.NftItemsCollectionPublic}>()
            ?? panic("Couldn't get collection")
        // We trust nftItems.Collection.borowNftItem to get the correct itemID
    ...
    init() {
        ...
        // Set our named paths
        self.CollectionStoragePath = /storage/nftItemsCollectionV14
        self.CollectionPublicPath = /public/nftItemsCollectionV14
        self.MinterStoragePath = /storage/nftItemsMinterV14
